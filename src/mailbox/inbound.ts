...
export type InboundEventResult =
  | { kind: "received"; id: string | null; pushSent: boolean }
  | { kind: "tracking"; status: string | null }
  | { kind: "ignored" }
  | { kind: "error"; error: "missing_from_to" };

export function eventToSentStatus(type: string): string | null {
...
export function resendEmailIdOf(data: Record<string, unknown>): string {
...
async function isSpamSender(db: MailboxDb, config: MailboxConfig, address: string): Promise<boolean> {
  const { data } = await db
...
export type HandleInboundOptions = {
  ...
  push?: { send: PushSender; options?: MailPushOptions; title: (assigned: boolean) => string };
};
...
export async function handleResendWebhookEvent(
  ...
  if (eventType === "email.received" || (!eventType && typeof data.from === "string")) {
    ...
    const inReplyTo =
      normalizeMessageId(data.in_reply_to) ??
    ...
    const insertedId = (insertedEmail?.id as string | undefined) ?? null;
    let pushSent = false;

    if (options.push) {
      pushSent = await sendMailPushNotification(
        db,
        config,
        options.push.send,
        assignment.notifyProfileIds,
        {
          type: "mail",
  ...
  if (!resendEmailId || !eventType) return { kind: "ignored" };
