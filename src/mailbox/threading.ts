 ...
 */
export function fallbackThreadKey(input: { subject: string; addresses: string[]; noSubjectLabel?: string }): string {
  const subject = normalizeSubjectForThread(input.subject) || (input.noSubjectLabel ?? "(nessun oggetto)");
  const participants = Array.from(
    new Set(input.addresses.map((address) => normalizeAddress(address)).filter(Boolean)),
  ).sort();
  return `fallback:${createHash("sha256").update(`${subject}|${participants.join("|")}`).digest("hex")}`;
}

export function generateOutboundMessageId(domain: string): string {
...
export async function resolveConversationThreadKey(
  ...
  for (const table of [config.tables.inbound, config.tables.sent]) {
    ...
    if (data?.thread_key) return data.thread_key as string;
  }
