 ...
 */
import { attachThreadMessages, hydrateInboundAssignments, hydrateMissingInboundBodies, } from "./hydration.js";
import { matchesMailSearch, pageMessages } from "./search.js";
export function parseMailboxView(value) {
    return value === "unread" ||
        value === "starred" ||
        value === "archived" ||
        value === "spam" ||
...
export async function loadMailbox(db, config, input) {
    ...
    let query = db
        ...
        .range(rangeStart, rangeEnd);
    if (view === "unread")
        query = query.eq("read", false).eq("archived", false).eq("spam", false);
    if (view === "starred")
        query = query.eq("starred", true).eq("spam", false);
    if (view === "archived")
        query = query.eq("archived", true).eq("spam", false);
    if (view === "spam")
        query = query.eq("spam", true);
    if (view === "inbox")
        query = query.eq("archived", false).eq("spam", false);
    ...
    if (error)
        throw error;
    const hydrated = await hydrateMissingInboundBodies(db, config, (data ?? []), input.resendApiKey);
    const filtered = isSearching
    ...
    return {
        ...
        page,
        pageSize,
        messages: await attachThreadMessages(db, config, withAssignees.map((message) => ({ ...message, source: "inbound" }))),
        total: isSearching ? filtered.length : count ?? 0,
