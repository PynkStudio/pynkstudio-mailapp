...
import { revokeMailPushNotification } from "./push.js";
const MAIL_MESSAGE_ACTIONS = [
    "read",
    "unread",
    "star",
    "unstar",
...
export function isMailMessageAction(value) {
    return typeof value === "string" && MAIL_MESSAGE_ACTIONS.includes(value);
}
function revokesPush(action) {
    return action === "read" || action === "archive" || action === "spam" || action === "delete";
}
async function revokePushIfNeeded(db, config, id, action, options) {
    if (!options.push || !revokesPush(action))
        return;
    ...
    if (!message)
        return;
    await revokeMailPushNotification(db, config, options.push.send, message, options.push.options).catch((pushError) => {
        console.error("[mailbox/message] push revocation failed", pushError);
    });
}
export async function applyMailMessageAction(db, config, id, action, options = {}) {
