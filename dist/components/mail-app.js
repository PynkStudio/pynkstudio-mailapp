...
export function MailApp({ initialInbox, initialSent, unreadTotal, unreadMine, currentSiteadminId, canCompose, mode = "platform", scope, tenantId, tenantName, tenantFromAddress, currentUserEmail, }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "menuary-admin-card overflow-hidden p-0", children: _jsxs("div", { className: "flex h-[calc(100vh-4rem)] min-h-[640px] rounded-[inherit] bg-white/45 backdrop-blur-xl max-lg:h-[calc(100vh-5rem)]", children: [_jsx("div", { className: "hidden lg:flex", children: _jsx(MailSidebar, { view: view, brand: brand, unreadCount: unread, unreadMine: unreadMyCount, canCompose: canCompose, mode: mode, mineAvailable: mineAvailable, onViewChange: handleViewChange, onBrandChange: handleBrandChange, onCompose: openBlankCompose, onOpenDeviceSettings: mode === "tenant" && tenantId ? () => setDeviceSettingsOpen(true) : undefined }) }), _jsxs("div", { className: cn("flex h-full flex-col overflow-hidden border-r border-black/10 bg-[var(--ma-surface)]/55", showDetail ? "hidden lg:flex lg:w-72 xl:w-80" : "flex-1"), children: [_jsxs("div", { className: "flex items-center justify-between border-b border-black/10 bg-white/45 px-4 py-2.5 backdrop-blur-xl", children: [_jsxs("div", { className: "flex flex-wrap gap-1 lg:hidden", children: [["inbox", "unread", "mine", "sent", "starred", "spam", "archived"]
                                                    ...
                                                    .map((v) => (_jsx("button", { onClick: () => handleViewChange(v), className: cn("rounded-lg px-2.5 py-1 text-xs font-medium transition-colors capitalize", view === v
                                                        ...
                                                        : "bg-[var(--ma-surface)] text-[var(--ma-muted)]"), children: v === "inbox" ? "Arrivo" : v === "unread" ? "Non lette" : v === "mine" ? "Le mie" : v === "sent" ? "Inviata" : v === "starred" ? "Stellate" : v === "spam" ? "Spam" : "Archivio" }, v))), mode === "platform" && ["all", "pynkstudio", "menuary", "bizery", "orpheo", "support"].map((b) => (_jsx("button", { onClick: () => handleBrandChange(b), className: cn("rounded-lg px-2.5 py-1 text-xs font-medium transition-colors", brand === b
                                                        ? "bg-[var(--ma-line)] text-[var(--ma-ink)]"
                                                        : "bg-[var(--ma-surface)] text-[var(--ma-muted)]"), children: b === "all" ? "Tutte" : b === "pynkstudio" ? "Pynk" : b === "menuary" ? "Menuary" : b === "bizery" ? "Bizery" : b === "orpheo" ? "Orpheo" : "Supporto" }, b)))] }), _jsxs("p", { className: "hidden text-xs font-medium text-[var(--ma-muted)] lg:block", children: [isSentView ? sent.total : `${inboundThreads.length} thread`, " \u00B7 ", isSentView ? "email" : `${inbox.total} email`] }), _jsxs("div", { className: "flex items-center gap-1", children: [mode === "tenant" && tenantId && (_jsx("button", { onClick: () => setDeviceSettingsOpen(true), className: "menuary-admin-nav-link !w-auto !p-1.5 lg:hidden", title: "Questo dispositivo", children: _jsx(Settings, { size: 13 }) })), _jsx("button", { onClick: () => reload(), disabled: isPending, className: "menuary-admin-nav-link !w-auto !p-1.5", title: "Aggiorna", children: _jsx(RefreshCw, { size: 13, className: isPending ? "animate-spin" : "" }) })] })] }), _jsx("div", { className: "flex-1 overflow-y-auto", children: _jsx(EmailList, { emails: listEmails, selectedId: selectedListId, onSelect: isSentView ? handleSelectSent : handleSelectInbound, trackingMap: isSentView ? sentTrackingMap : undefined, threadCountMap: isSentView ? undefined : threadCountMap, threadUnreadMap: isSentView ? undefined : threadUnreadMap, threadAttachmentMap: isSentView ? undefined : threadAttachmentMap }) })] }), showDetail ? (_jsx("div", { className: "flex-1 overflow-hidden", children: isSentView && selectedSent ? (_jsx(SentDetail, { email: selectedSent, onClose: () => setSelectedSent(null) })) : selectedInbound ? (_jsx(EmailDetail, { email: selectedInbound, onClose: () => setSelectedInbound(null), onMutated: () => reload(), onReply: handleReply, onAssigned: handleAssigned, mode: mode, scope: scope, threadEmails: threadByLatestId[selectedInbound.id] ?? [selectedInbound] })) : null })) : (_jsx("div", { className: "hidden flex-1 items-center justify-center text-[var(--ma-muted)] lg:flex", children: _jsxs("div", { className: "text-center", children: [_jsx(Mail, { size: 36, className: "mx-auto mb-2 opacity-25" }), _jsx("p", { className: "text-sm", children: "Seleziona un'email" })] }) }))] }) }), canCompose && (_jsx("button", { onClick: openBlankCompose, className: "menuary-admin-action-btn fixed bottom-6 right-6 z-40 flex items-center gap-2 shadow-lg lg:hidden", children: "\u270F\uFE0F Scrivi" })), _jsx(ComposeDrawer, { open: composeOpen, canCompose: canCompose, defaultBrand: composePrefill.brand ?? composeBrandFromFilter(brand), tenantId: tenantId, fromAddress: tenantFromAddress, fromName: tenantName, currentUserEmail: currentUserEmail, lockBrand: mode === "tenant", preferredFromAddress: composePrefill.preferredFromAddress, initialTo: composePrefill.to, initialSubject: composePrefill.subject, initialBody: composePrefill.body, initialAttachments: composePrefill.attachments, onClose: () => setComposeOpen(false), onSent: () => { if (view === "sent")
                    reload("sent", brand); } }), mode === "tenant" && tenantId && (_jsx(TenantMailDeviceSettings, { open: deviceSettingsOpen, tenantId: tenantId, onClose: () => setDeviceSettingsOpen(false), onFilterChange: handleDeviceFilterChange }))] }));
}
